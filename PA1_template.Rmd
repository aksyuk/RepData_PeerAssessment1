...
Missing values are to be replaced with medians in corresponding time interval, using key column available in data.table objects. Data with filled gaps is saved in table **DT.filled**. Table **DT.med.perint** contains medians calculated for each 5 minute interval. **'Interval'** is a key variable both in **DT.filled** and **DT.med.perint**.
...
# calculate medians
DT.med.perint <- DT[, c('steps', 'interval'), with  = F][, lapply(.SD, median, na.rm = T), by = interval]
# data.table preview
DT.med.perint[interval == 900]

# column 'interval' is a key
setkey(DT.filled, interval)
setkey(DT.med.perint, interval)

# NA = median by interval
DT.filled[is.na(steps), 'steps'] <- DT.med.perint[, 'steps', with = F]
# data.table preview
head(DT.filled[interval == 900])
...
Filling missing values slightly affected on mean and median:
...
* Mean and median now differ and belong to different intervals of histogram.
...
* On the contrary, intervals from 05:00 to 09:00 and from 18:00 to 21:00 show more activity. Moreover, as curve increases in morning hours and falls in evening hours, one can assume that way from home to work and back again may be the leading factor here.
